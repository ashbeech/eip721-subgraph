...
export function handleTransfer(event: Transfer): void {
    ...
    if (from != zeroAddress || to != zeroAddress) { // skip if from zero to zero
        ...
        if (from != zeroAddress) { // existing token
            ...
            // 'from' == zeroAddress so token user minting = intial 'to'
            minterID = true;
        }
        
        if(to != zeroAddress) { // transfer (can be intitial transfer AKA mint)
            // owner
            let newOwner = Owner.load(to);
            if (newOwner == null) {
                newOwner = new Owner(to);
                newOwner.numTokens = BigInt.fromI32(0);
                newOwner.numMints = BigInt.fromI32(0);
            }
            // token
            ...
            if(eip721Token == null) {
                ...
                if(minterID == true){
                    ...
                    eip721Token.minter =  to; // minter set once when 'from' = zeroAddress
                }
            ...
            if (from == zeroAddress) { // mint +1
                ...
                newOwner.numMints = newOwner.numMints.plus(BigInt.fromI32(1));
            }
